...
//建立关键字树
  public Nodea buildkeywordtree(int i,int r,Sys sys){
    Nodea trie = new Nodea(); //创建一个新关键字树（树根）
    trie.ID = 0; //树根的ID为0

    for (int j = 0; j < Pi[i].length()/ r; j++) //建树
     { //对于每一个长度为r的划分子串

        sys.build(Pi[i], j * r, r, j + 1, trie); //根据这个子串建树
     }
    sys.failLink(trie); //建立整个树的失效链接
    return(trie);
  }
  //计算划分的长度r
  public int findr(){           //########有待修改#############
    int rr;
    if(Pi[1].length()<300)
      rr=Pi[1].length();
    else
  ...
  public int findCenterSeq(){
    ...
    for(int i=0;i<n;i++)
    {
      Nodea keywordtreefori=new Nodea();
      keywordtreefori=buildkeywordtree(i,r,sys);
      for(int j=0;j<n;j++)
      {
        if(j==i)
          continue;
        sys.ACsearch(Pi[j], keywordtreefori);
        Name[i][j]=sys.out();
  ...
  public void prealign(int i){
       if(Name[center][i][0][0]!=0&&Name[center][i][1][0]!=0)
         { String strC=Pi[center].substring(0,(Name[center][i][0][0]-1)*r);//此处有漏洞，如果Name[center][i][0][0]=0，会抱错
  ...
  public void midalign(int i,int j){
    if(Name[center][i][0][j]!=0&&Name[center][i][1][j]>=Name[center][i][1][j-1]+r&&Name[center][i][0][j]-1>=Name[center][i][0][j-1])
    {
      String strC=Pi[center].substring(Name[center][i][0][j-1]*r,(Name[center][i][0][j]-1)*r);//此处有漏洞，如果Name[center][i][0][0]=0，会抱错
      String stri=Pi[i].substring(Name[center][i][1][j-1]+r-1,Name[center][i][1][j]-1);
      int M[][]=new int[stri.length()+1][strC.length()+1];   //定义动态规划矩阵
      M=computeScoreMatrixForDynamicProgram(stri,strC);//动态规划矩阵计算完毕
      traceBackForDynamicProgram(M,stri.length(),strC.length(),i,Name[center][i][1][j-1]+r-1,Name[center][i][0][j-1]*r);
    }
  }
  //比对Pi[i]与中心序列有重合的后端
